// Lógica do jogo integrada com Supabase
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function init() {
        ...
        if (window.mercadoPagoPayments) {
            await window.mercadoPagoPayments.initialize();
        }
    }
    ...
    async function startNewGame(betAmount) {
        ...
        console.log('Número sorteado:', targetNumber); // Para fins de teste
    ...
    function updateBalance() {
        if (userBalanceElement) {
            userBalanceElement.textContent = `R$ ${userBalance.toFixed(2)}`;
        }
    }
    ...
    if (!window.mercadoPagoPayments) {
        alert('O sistema de pagamentos não está disponível.');
        return;
    }
